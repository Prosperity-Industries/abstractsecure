...
export const validateWebhookPayload = (data: any): ValidationError[] => {
  ...
  if (!personalInfo) {
    ...
    if (!personalInfo.fullName) errors.push({ field: 'fullName', message: 'Full name is required' });
    if (!personalInfo.dateOfBirth) errors.push({ field: 'dateOfBirth', message: 'Date of birth is required' });
    if (!personalInfo.ssn) {
    ...
    if (!personalInfo.maritalStatus) errors.push({ field: 'maritalStatus', message: 'Marital status is required' });
    if (!personalInfo.roleInTransaction) errors.push({ field: 'roleInTransaction', message: 'Role in transaction is required' });
  }
  ...
  if (!propertyInfo) {
    ...
    if (!propertyInfo.address) errors.push({ field: 'address', message: 'Property address is required' });
    if (!propertyInfo.titleOrderNumber) errors.push({ field: 'titleOrderNumber', message: 'Title order number is required' });
  }

  // Validate additional parties if present
  if (data['1'].additionalParties) {
    data['1'].additionalParties.forEach((party: any, index: number) => {
      if (!party.name) errors.push({ field: `additionalParties[${index}].name`, message: 'Additional party name is required' });
      if (!party.dateOfBirth) errors.push({ field: `additionalParties[${index}].dateOfBirth`, message: 'Additional party DOB is required' });
