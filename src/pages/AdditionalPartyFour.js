...
const AdditionalPartyFour = () => {
    const navigate = useNavigate();
    const { toast } = useToast();
    const role = localStorage.getItem('roleInTransaction') || '';
    const partyType = role === 'buyer' ? 'Buyer' : 'Seller';
    const [formData, setFormData] = useState({
        fullName: localStorage.getItem('additionalParty4_fullName') || '',
        dateOfBirth: localStorage.getItem('additionalParty4_dateOfBirth') || '',
        ssn: localStorage.getItem('additionalParty4_ssn') || '',
        maritalStatus: localStorage.getItem('additionalParty4_maritalStatus') || '',
        hasMoreParties: localStorage.getItem('additionalParty4_hasMoreParties') || ''
    });
    const handleInputChange = (e) => {
        ...
        localStorage.setItem(`additionalParty4_${name}`, value);
    };
    const handleSelectChange = (value, field) => {
        ...
        localStorage.setItem(`additionalParty4_${field}`, value);
    };
